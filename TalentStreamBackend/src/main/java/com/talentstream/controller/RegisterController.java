...
//	    }
	    ...
	    @PostMapping("/applicant/resetpassword/{email}")
		...
		@PostMapping("/applicant/fogotpassword/sendotp")
	    public ResponseEntity<String> forgotPasswordsendOtp(@RequestBody ApplicantRegistration  request) {
	        String userEmail = request.getEmail();
	        ApplicantRegistration applicant = regsiterService.findByEmailAddress(userEmail);
	        if (applicant != null) {     
	            String otp = otpService.generateOtp(userEmail);
